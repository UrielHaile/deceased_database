...
class fetchingData():
    def __init__(self):
        ...
        self.page_to=requests.get("http://www.dgis.salud.gob.mx/contenidos/basesdedatos/da_defunciones_gobmx.html")
        self.soup=BeautifulSoup(self.page_to.text,'html.parser')
        self.tables=self.soup.findAll("table")
        self.name=[h3.get_text().replace(' ','_')  for h3 in self.soup.findAll("h3")]
    ...
    def unzip(self,path_zip):
        with zipfile.ZipFile(path_zip, 'r') as zip_ref:
            zip_ref.extractall(os.path.dirname(path_zip))  # Extrae en el mismo directorio que el ZIP
